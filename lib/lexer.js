...
(function() {
  var BOM, BOOL, CALLABLE, CODE, COFFEE_ALIASES, COFFEE_ALIAS_MAP, COFFEE_KEYWORDS, COMMENT, COMPARE, COMPOUND_ASSIGN, HEREDOC, HEREDOC_ILLEGAL, HEREDOC_INDENT, HEREGEX, HEREGEX_OMIT, IDENTIFIER, INDEXABLE, INVERSES, JSTOKEN, JS_FORBIDDEN, JS_KEYWORDS, LINE_BREAK, LINE_CONTINUER, LOGIC, Lexer, MATH, MULTILINER, MULTI_DENT, NOT_REGEX, NOT_SPACED_REGEX, NUMBER, OPERATOR, REGEX, RELATION, RESERVED, Rewriter, SHIFT, SIMPLESTR, STRICT_PROSCRIBED, TRAILING_SPACES, UNARY, WHITESPACE, compact, count, invertLiterate, key, last, locationDataToString, ref, ref1, repeat, starts, throwSyntaxError,
  ...
  exports.Lexer = Lexer = (function() {
    ...
    Lexer.prototype.regexToken = function() {
      var flags, length, match, prev, ref2, ref3, regex;
      ...
      if (prev && (ref2 = prev[0], indexOf.call((prev.spaced ? NOT_REGEX : NOT_SPACED_REGEX), ref2) >= 0)) {
        return 0;
      }
      if (!(match = REGEX.exec(this.chunk))) {
        return 0;
      }
      ref3 = match, match = ref3[0], regex = ref3[1], flags = ref3[2];
      if (regex.slice(0, 2) === '/*') {
    ...
    Lexer.prototype.heregexToken = function(match) {
      var body, flags, flagsOffset, heregex, j, len1, plusToken, prev, re, ref2, ref3, ref4, tag, token, tokens, value;
      ...
      tokens = [];
      ref2 = this.interpolateString(body, {
        regex: true
      });
      ...
      tokens.pop();
      if (((ref3 = tokens[0]) != null ? ref3[0] : void 0) !== 'STRING') {
        this.token('STRING', '""', 0, 0);
        this.token('+', '+', 0, 0);
      }
      (ref4 = this.tokens).push.apply(ref4, tokens);
      if (flags) {
    ...
    Lexer.prototype.lineToken = function() {
      var diff, indent, match, newlines, noNewlines, size;
      ...
      if (size > this.indent) {
        ...
        this.indebt = 0;
        this.outdentToken(this.indent - size, noNewlines, indent.length, newlines);
      }
      this.indent = size;
      return indent.length;
    };

    Lexer.prototype.outdentToken = function(moveOut, noNewlines, outdentLength, lines) {
      var dent, len;
      while (moveOut > 0) {
      ...
      if (!(this.tag() === 'TERMINATOR' || noNewlines)) {
        this.token('TERMINATOR', lines, outdentLength, 0);
      }
      return this;
    };
    ...
    Lexer.prototype.newlineToken = function(offset, lines) {
      if (lines == null) {
        lines = 1;
      }
      while (this.value() === ';') {
      ...
      if (this.tag() !== 'TERMINATOR') {
        this.token('TERMINATOR', lines, offset, 0);
      }
      return this;
    };
    ...
    Lexer.prototype.literalToken = function() {
      var match, prev, ref2, ref3, ref4, ref5, tag, value;
    ...
    Lexer.prototype.sanitizeHeredoc = function(doc, options) {
      var attempt, herecomment, indent, match, ref2;
    ...
    Lexer.prototype.balancedString = function(str, end) {
      var continueCount, i, j, letter, match, prev, ref2, stack;
      continueCount = 0;
      stack = [end];
      for (i = j = 1, ref2 = str.length; 1 <= ref2 ? j < ref2 : j > ref2; i = 1 <= ref2 ? ++j : --j) {
        if (continueCount) {
          --continueCount;
          continue;
        }
    ...
    Lexer.prototype.interpolateString = function(str, options) {
      var column, expr, heredoc, i, inner, interpolated, j, len, len1, letter, lexedLength, line, locationToken, nested, offsetInChunk, pi, plusToken, popped, ref2, ref3, ref4, regex, rparen, strOffset, tag, token, tokens, value;
      if (options == null) {
        options = {};
      }
      ...
      while (letter = str.charAt(i += 1)) {
        ...
        if (inner.length) {
          ref2 = this.getLineAndColumnFromChunk(strOffset + i + 1), line = ref2[0], column = ref2[1];
          ...
          popped = nested.pop();
          if (((ref3 = nested[0]) != null ? ref3[0] : void 0) === 'TERMINATOR') {
            popped = nested.shift();
          }
      ...
      for (i = j = 0, len1 = tokens.length; j < len1; i = ++j) {
        token = tokens[i];
        tag = token[0], value = token[1];
        if (i) {
          if (i) {
        ...
        if (tag === 'TOKENS') {
          (ref4 = this.tokens).push.apply(ref4, value);
        } else if (tag === 'NEOSTRING') {
          token[0] = 'STRING';
    ...
    Lexer.prototype.unfinished = function() {
      var ref2;
      return LINE_CONTINUER.test(this.chunk) || ((ref2 = this.tag()) === '\\' || ref2 === '.' || ref2 === '?.' || ref2 === '?::' || ref2 === 'UNARY' || ref2 === 'MATH' || ref2 === '+' || ref2 === '-' || ref2 === 'SHIFT' || ref2 === 'RELATION' || ref2 === 'COMPARE' || ref2 === 'LOGIC' || ref2 === 'THROW' || ref2 === 'EXTENDS');
    };
